...
let lines = fs.readFileSync(path.join(__dirname, './lines.ont'), 'utf8').toString().split(/\n/)
...
index.forEach((item, i) => {
    ...
    */
    ...
    // let indexRow = books[number].code + ' ' + ind.c + ':' + ind.v + ' <==========='
    // out.push(indexRow)
    ...
    if (indexBook !== ind.b) {
        ...
        indexBook = ind.b
        let bookRow = '  <b n="' + books[number].long + '">'
        out.push(bookRow)
    }

    if (indexChapter !== ind.c) {
        if (indexChapter !== 0) {
            out.push('    </c>')
        }
        indexChapter = ind.c
        let chapterRow = '    <c n="' + ind.c + '">'
        out.push(chapterRow)
    }

    let row = '      <v n="' + ind.v + '">' + text + '</v>'
    out.push(row)
})
